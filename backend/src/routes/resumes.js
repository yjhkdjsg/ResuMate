...
async function loadVersion(resumeId, versionId) {
  if (!resumeId || !versionId) {
    throw ApiError.badRequest("Resume ID and Version ID are required");
  }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const selected = req.body.rewriteIds?.length
      ? analysis.bulletRewrites.filter((rewrite) =>
          req.body.rewriteIds.includes(rewrite._id.toString())
        )
